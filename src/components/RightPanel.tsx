...
/**
 * Right panel component displaying chapter summary and notes.
 * 
 * Features:
 * - Collapsible panel with smooth animations
 * - Chapter summary display with markdown rendering
 * - Chapter notes with custom markdown components
 * - Ornamental separator between sections
 * - Auto-scroll to top on chapter change
 * - Custom scrollbar styling
 * 
 * @component
 * @returns {JSX.Element} Right sidebar with summary and notes
 */
const RightPanel: React.FC = () => {
  ...
  return (
    <aside className={`h-full border-l transition-all duration-300 ease-in-out ${isExpanded ? 'w-[300px]' : 'w-[40px]'} flex flex-col`}>
      ...
      <div ref={scrollContainerRef} className={`flex-1 px-4 py-2 overflow-y-auto transition-all duration-300 ease-in-out scrollbar-hide ${isExpanded ? 'opacity-100' : 'opacity-0 pointer-events-none'
        }`}>
